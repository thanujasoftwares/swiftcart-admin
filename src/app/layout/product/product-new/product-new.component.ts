...
export class ProductNewComponent implements OnInit {
  ...
  constructor(
    ...
    // this.subcategories = menuService.subcategories;
    this.materials=['cotton','silk','linen','wool','leather','ramie','hemp','jute','acetate','chiffon','acrylic','organza',
  ...
  add(){
    this.productService.add(this.newproduct).subscribe((product)=>{
      console.log(product);
      if(product){
        console.log("going to ",product.id);
        this.router.navigate(['/products/'+product.id],{queryParams:{n1:1}});
      }
    },(err)=>{
      console.log(err);
    })
